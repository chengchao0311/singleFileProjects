...
for($i=0, $size=count($news);$i<$size;$i++){
	if($news[$i]["play_url"] != ""){
        $herf = '<a href="'.$news[$i]["play_url"].' target="_blank">'.$news[$i]["video_name"].'</a><br>';
	}else{
        $herf = '<a href="'.$news[$i]["video_reurl"].' target="_blank"><font color="black">'.$news[$i]["video_name"].'</font></a></font><br>';
	}
        echo $herf;
}
